...
function sd_woocommerce_review_order_before_order_total()
{
    ...
    if (!empty($list_cart_items)) {
        foreach ($list_cart_items as $item) {
            ...
            Ship_Depot_Logger::wrlog('[sd_woocommerce_review_order_before_order_total] item qty: ' . print_r($item_quantity, true));
            ...
            $package_size->Weight = isset($product_data['weight']) ? Ship_Depot_Helper::ConvertToShipDepotWeight($product_data['weight']) * $item_quantity : 0;
            $package_size->Quantity = 1;
            //Update 1.2.18: calculate package size with quantity
            if ($item_quantity > 1) {
                $min_value = min($package_size->Length, $package_size->Width, $package_size->Height);
                if ($min_value == $package_size->Width) {
                    $package_size->Width = $package_size->Width * $item_quantity;
                } else if ($min_value == $package_size->Length) {
                    $package_size->Length = $package_size->Length * $item_quantity;
                } else {
                    $package_size->Height = $package_size->Height * $item_quantity;
                }
            }
            Ship_Depot_Logger::wrlog('[sd_woocommerce_review_order_before_order_total] package_size: ' . print_r($package_size, true));
...
function sd_action_checkout_order_processed($order_id, $posted_data, WC_Order $order)
{
    ...
    foreach ($list_items as $item) {
        ...
        $package_size->Weight = Ship_Depot_Helper::ConvertToShipDepotWeight($product->get_weight()) * $item_quantity;
        $package_size->Quantity = 1;
        //Update 1.2.18: calculate package size with quantity
        if ($item_quantity > 1) {
            $min_value = min($package_size->Length, $package_size->Width, $package_size->Height);
